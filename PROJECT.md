...
## Где что (контейнер `imyavel/` = просто папка с клонами)
| Путь | Что | Публикация |
|---|---|---|
| **`yaniktoim/`** | этот репо: сайт `docs/` + движок `cms-revival/` + доки | `imyavel.github.io/yaniktoim/` (Pages: `main`, папка `/docs`) |
| `imyavel.github.io/` | корневой лендинг (отд. репо) | `imyavel.github.io` |
| `zohar-sulam/` | перевод Зохара (отд. репо) | под общим доменом; **ZML/CMS его не касаются** |
| `imyavel/push.bat`,`pull.bat` | деплой/синк трёх репо (локальные у оператора, в git не хранятся) | — |

Pages отдаёт только `docs/`. Всё вне `docs/` (`cms-revival/`, `PROJECT.md`, `tools/`, `manifest.json`, roadmap'ы)
...
**ZML** (свой текстовый формат). Каждая статья в двух видах: старая LLM-вёрстка `.html` и ZML-рендер
`.view.html`; по умолчанию показывается старая, со ссылкой «Новая версия (ZML)». Правка — прямо на сайте
...
- **`editor/`** — `render.js` (+ `verse_split.js`, `faw_markup.js`, шаблоны `data/template_*.html`) и сборщики.
  `node cms-revival/editor/build_views.mjs [id…]` рендерит `.zml`→`.view.html` И синкает render+шаблоны+темы в `docs/`.
...
- **Правка на сайте**: статья — ✎ на `.view.html`; структура/новая статья — `structure.html` (admin); песни — `songs/index.view.html`. Всё → Worker → коммит.
...
# Остальные репо (кратко)
- **`imyavel.github.io/`** — заглавная домена (`index.html` + `modus-operandi/`); счётчик статей фетчит `/yaniktoim/config/structure.json`.
- **`zohar-sulam/`** — отдельный статичный сайт (перевод Зохара, главы по парашам). Свой конвейер/оркестратор; ZML/CMS его не касаются.
