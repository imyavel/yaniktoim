...
# Ожидающие роадмапы
- **`enrich_zohar.md`** (Фаза 1) — инфраструктура Зоар-ссылок. **ВЫПОЛНЕНА 2026-06-24.**
  `zohar-sulam` **запушен в живое** (`52e87fa`: split `bereshit`→`bereshit-1/2 (Вначале)`, якоря
  `#p§`, индексы-справочник, sitemap+pagefind). yaniktoim-инфра (`render.js`, `zohar_index.paras`,
  SPEC, guide, 29 ссылок корзины A) — в рабочем дереве, **НЕ запушена** (идёт Фаза 2).
- **`enrich_zohar_unify.md`** (Фаза 2) — **ОЖИДАЕТ ВЫПОЛНЕНИЯ.** Унификация ВСЕХ Зоар-упоминаний
  корпуса к тегу `{глава|N|§}` (номер статьи сохраняем; убрать рукописный текст книги/тома/статьи;
  §NN всегда; охватить и незалинкованные), затем сверка цитат с нашим переводом (HTML-отчёт на
  согласование). Самодостаточна для холодного старта. Отчёт-черновик правок Фазы 1 — `enrich_zohar_changes.md`.
