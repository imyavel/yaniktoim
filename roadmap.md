...
2. **Flip (он же первая публикация, необратимо):** удалить старую `C:\Users\admin\yaniktoim\`; переместить `yaniktoim-cms\` → `yaniktoim\` (вместе с `.git`); `git remote add origin https://github.com/imyavel/yaniktoim`; выверить `.gitignore` (см. 0.7); собрать в `docs/`; **force-push** нашей истории в `main` (затирает старое содержимое/историю репо); через `gh api` переключить Pages source на `main /docs`.
3. **Публикует оператор сам**: запускает `batch_runner/gui.pyw` из новой `yaniktoim\` → build + commit + push первой минимально рабочей версии (с inline-Edit). Если push должен заменить чужую историю — настроить так, чтобы это сработало (force на первом пуше).
4. Проверка Edit вживую — **после** переноса; баги отлаживаем на месте.
...
12. **[ ] Создать `src/build_art_ids.py`** — разово-запускаемый генератор арт-id для всех 350 статей. На входе: `manifest.json` + `raw/*.html` (для извлечения author-date из текста). На выходе: расширенный `manifest.json` с новым полем `art: "<id>"` для каждой записи. Алгоритм: см. SPEC §2.3. Падает при коллизиях. Идемпотентен. После прогона — `manifest.number` уходит в `_legacy_number` для архива.
13. **[~] Мини-CMS (локальная, MVP)** — `cms/` папка, stdlib-only HTTP сервер на localhost:8765. **СУПЕРСЕДНУТА** браузерным редактором (TODO 13c, 2026-05-29). Папка `cms/` пока оставлена, но не развивается; можно удалить после переноса нужного (transform-кнопка остаётся в батч-пайплайне).
